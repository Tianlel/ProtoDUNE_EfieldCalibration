...
void get_displacement_map()
{
    ...
    TFile *f = TFile::Open("/dune/app/users/tianlel/protoDUNE/E_field/ProtoDUNE_EfieldCalibration/output_ROOTtree/reco/frame_distortion/ALLEVENTS_with_thermal_unordered_dT.root", "READ");
...
/*
    TFile *file = new TFile("frameXoffset_unordered_dT_with_thermal.root", "recreate");
    out->Write();
    out_neg->Write();
    file->Write();
    file->Close();
