...
double X0 = 358.6; // cm
double driftv = 0.156461; //0.156461 cm/us 
...
float get_distortion_error(float err)
{
    return driftv*err/2.0;
}
...
void get_displacement_map1()
{
    ...
    TH2F *out = new TH2F("frameXoffset_pos","Xoffset (beam left); Z (cm); Y(cm); Xoffset (cm)", nbinsZ, Zmin, Zmax, nbinsY, Ymin, Ymax);
    TH2F *out_neg = new TH2F("frameXoffset_neg","Xoffset (beam right); Z (cm); Y(cm); Xoffset (cm)", nbinsZ, Zmin, Zmax, nbinsY, Ymin, Ymax);
    ...
    for (int i=0; i<nbinsZ; i++)
    {
        for (int j=0; j<nbinsY; j++)
        {
            ...
            if (med!=0)
            {
                out->SetBinContent(i+1,j+1,get_distortion(med,0));
                out->SetBinError(i+1,j+1,get_distortion_error(err));
            }
        }
    }
    for (int i=0; i<nbinsZ; i++)
    {
        for (int j=0; j<nbinsY; j++)
        {
            ...
            if (med!=0) 
            {
                out_neg->SetBinContent(i+1,j+1,get_distortion(med,1));
                out_neg->SetBinError(i+1,j+1,get_distortion_error(err));
            }
        }
    }
/*                          
    ...
    out->Draw("COLZ TEXT");
    out->GetZaxis()->SetRangeUser(-3.2,2);
  ...
  //  c1->SaveAs(save_name+".png");
    ...
    out_neg->GetZaxis()->SetRangeUser(-3.2,3);
*/
//    c2->SaveAs("frameDisplacementTH2_unordered_with_thermal_neg.png");
    TFile *file = new TFile("../../distortion_maps/frameXoffset_unordered_dT_without_thermal.root", "recreate");
    out->Write();
    out_neg->Write();
    file->Write();
    file->Close();
